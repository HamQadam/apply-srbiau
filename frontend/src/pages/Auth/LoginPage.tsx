...
export function LoginPage() {
  ...
  const handleRequestOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.debug_code) setDebugCode(response.debug_code);
      setStep('otp');
      ...
      <div className="min-h-screen flex items-center justify-center bg-background py-12 px-4">
        <div className="max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-text-muted">{t('auth.welcomeSubtitle')}</p>
          </div>
          ...
          <div className="bg-surface rounded-xl shadow-sm border border-border p-8">
            {step === 'phone' ? (
              ...
              <form onSubmit={handleVerifyOTP}>
                <div className="mb-2 text-sm text-text-muted">{t('auth.sentTo', { phone })}</div>
          ...
          <p className="mt-6 text-center text-sm text-text-muted">{t('auth.terms')}</p>
        </div>
      </div>
    </PageTransition>
  );
